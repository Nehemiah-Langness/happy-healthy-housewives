...
export function RecipeLink({ image, description, title, to }: { image: string; title: string; description: string, to: string }) {
    return (
        <Link to={to} style={{ cursor: 'pointer' }} className='rounded-3 overflow-hidden text-decoration-none d-flex flex-column flex-lg-row recipe-link gap-2'>
            <RecipeLinkImage image={image} />
            <div className='d-flex flex-column gap-2 px-3 py-1'>
                <h1 className='dancing-script text-dark'>{title}</h1>
